...
def add_assessment():
    ...
    if form.validate_on_submit():  # Check if the form is submitted and valid
        ...
        try:
            ...
            assessment = new_assessment  # Set the assessment variable to the newly created assessment
        except Exception as e:
            db.session.rollback()  # Rollback in case of error
            flash(f'Error adding assessment: {e}')
    
    return render_template('add_assessment.html', form=form, assessment=assessment)  # Render the manage template with the form and assessment
